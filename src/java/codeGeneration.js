...
export function generateJavaSource(context) {
  const generator = new CodeGenerator(context);

  generator.printOnNewline('//  This file was automatically generated and should not be edited.');
  generator.printNewline();
  generator.printOnNewline('package com.lobsterink.campus.components.network;'); // TODO : other import?
...
export function classDeclarationForOperation(
  ...
  classDeclaration(generator, {
    ...
    if (source) {
      generator.printOnNewline('public static final String operationDefinition =');
    ...
    if (fragmentsReferenced && fragmentsReferenced.length > 0) {
      generator.printOnNewline('private final String queryDocument = operationDefinition');
      fragmentsReferenced.forEach(fragment => {
        generator.print(`.concat(${typeNameForFragmentName(fragment)}.fragmentDefinition)`)
      });
    }
    ...
    if (variables && variables.length > 0) {
      ...
      generator.printNewlineIfNeeded();
      initializerDeclarationForProperties(className, generator, properties);
      generator.printNewlineIfNeeded();
      mappedProperty(generator, { propertyName: 'variables', propertyType: 'String' }, properties);
    }
...
export function initializerDeclarationForProperties(className, generator, properties) {
  generator.printOnNewline(`public ${className}`);
  generator.print('(');
  generator.print(join(properties.map(({ propertyName, fieldType }) =>
    join([
      `${typeNameFromGraphQLType(generator.context, fieldType)} ${propertyName}`,
  ...
  generator.withinBlock(() => {
    properties.forEach(({ propertyName }) => {
      generator.printOnNewline(`this.${propertyName} = ${propertyName}`);
    });
  });
}

export function mappedProperty(generator, { propertyName, propertyType }, properties) {
  generator.printOnNewline(`public ${propertyType} get${propertyName}() `);
  generator.withinBlock(() => {
